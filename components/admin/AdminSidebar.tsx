...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ isOpen = false, onClose }) => {
    ...
    return (
        <>
            ...
            <div className={`
                w-64 bg-deep-blue text-cream flex flex-col h-full fixed top-0 left-0 shadow-lg z-50
                transition-transform duration-300 ease-in-out
                ${isOpen ? 'translate-x-0' : '-translate-x-full'}
                md:translate-x-0
            `}>
                <div className="p-4 border-b border-cream/20 flex justify-between items-center">
                    <Link to="/admin" className="text-2xl font-serif font-bold text-center block">Admin Panel</Link>
                    {/* Mobile Close Button */}
                    <button onClick={onClose} className="md:hidden text-cream/80 hover:text-cream">
